...
TT_IDENTIFIER = "IDENTIFIER" # Var identifier
...
class Error:
    ...
    def as_string(self):
        ...
        result += "\n\n" + \
            error_repr.error_repr(self.pos_start.ftxt,
...
class RTError(Error):
    ...
    def as_string(self):
        ...
        result += f'{self.error_name}: {self.details}'
        result += '\n\n' + \
            error_repr.error_repr(self.pos_start.ftxt, self.pos_start, self.pos_end)
        return result
...
class Parser:
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        "Expected ')', 'var', 'if', 'for', 'while', 'function', integer, float, identifier, '+', '-', '(' or 'not'"
                    ))
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(
            tok.pos_start,
            tok.pos_end,
            'Expected Integer, Float, Identifier, "if", "for", "while", "function", "+", "-" or "(", "["' 
        ))
    ...
    def expr(self):
        ...
        if res.error: 
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                'Expected "Variable", "Integer", "Float", "Identifier", "if", "for", "while", "function", "+", "-"  or "(", "["'
            ))
    ...
    def list_expr(self):
        ...
        if self.current_tok.type == TT_RSQUARE:
            ...
            if res.error:
                return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected ']', 'var', 'ig', 'for', 'while', 'function', integer, float, identifier, '+', '-', '(', '[' or 'NOT'"
                ))
    ...
    def comp_expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                'Expected Integer, Float, Identifier,"+", "-", "not" or "(", "["' 
            ))
...
BuiltInFunction.print_ret = BuiltInFunction("printr")
...
class SymbolTable:
    def __init__(self, parent = None):
        self.symbols = {}
        self.parent = parent # Global Variables :)
